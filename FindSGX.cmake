    ...
    function(enclave_sign target)
        ...
        if(SGX_HW AND SGX_MODE STREQUAL "Release")
            add_custom_target(${target}-sign ALL 
                              COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --green "The enclave is built in ${SGX_MODE} mode and hardware ${SGX_HW}"
                              COMMAND ${SGX_ENCLAVE_SIGNER} gendata 
                              -enclave $<TARGET_FILE:${target}> 
                              -config ${CONFIG_ABSPATH}
                              -out $<TARGET_FILE_DIR:${target}>/${target}_hash.hex
                              ${IGN_INIT} 
                              ${IGN_REL}
                              COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --cyan "SGX production enclave first step signing finished, use ${CMAKE_CURRENT_BINARY_DIR}/${target}_hash.hex for second step" 
                              WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})   
        else()
            add_custom_target(${target}-sign ALL 
                              COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --green "The enclave is built in ${SGX_MODE} mode and hardware ${SGX_HW}"
                              ...
                              COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --cyan "The enclave has been signed with test private key."
                              WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})
        endif()
