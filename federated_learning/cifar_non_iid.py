...
client_images = np.load("clients_images_cifar_noniid.npy", allow_pickle=True)
client_labels = np.load("clients_labels_cifar_noniid.npy", allow_pickle=True)

test_images = np.load("images_cifar.npy")
test_labels = np.load("labels_cifar.npy")
...
def fl_collude_byzantine_attacker(gf, round, aggregation_method, m, r, k, weight):
  model_accuracy_array = []
  model_error_array = []
  tmp_weight = weight
  acc_filename = "result_data/cifar/byzantine_attacker_collude/acc_noniid2"+aggregation_method
  err_filename = "result_data/cifar/byzantine_attacker_collude/err_noniid2"+aggregation_method
  ...
  # byzantine_weight_list = []
  # for i in range(gf):
  #   byzantine_weight_list.append(byzantine_weight)
  for x in range(round):
    ...
    #   i=i+1
    # byzantine_weight = tmp_weight
    # byzantine_weight[1][0] = 10000

    # byzantine_weight_list = []
    # for i in range(gf):
    #   weights.append(byzantine_weight)

    # weights.extend(byzantine_weight_list)
    if (aggregation_method == "average"):
      ...
      tmp_weight = aggregation_sar2(weights, n, gf, r, k)
    del weights_list
    del weights
...
def main(argv):
  ...
  for opt, arg in opts:
    ...
    #这是没有攻击的情况
  gf = int(f)
  fl_collude_byzantine_attacker(gf,round, method, int(m), int(float(r)* total_parameters), int(k), tmp_weight)
