...
def aggregation_trimed_mean(model_weights, beta):
    ...
    for i in range(layerNum):
        ...
        for j in range(number):
            flattened_weight.append(model_weights[j][i].flatten())
        # 每一层展开后加入到weight中，这时weight是一个二维数组，对每一列进行排序然后选择
        flattened_weight = np.array(flattened_weight)
        num = len(flattened_weight[0])
        tmp = np.zeros(num, dtype=np.float32)
        for j in range(num):
            column = flattened_weight[:,j]
            column.sort()
...
ray_images, ray_labels = process_data_to_ray(client_images[0:100-20], client_labels[0:100-20], workers)
...
def fl_non_collude_byzantine_attacker(gf, round, aggregation_method, m, r, k, weight):
  model_accuracy_array = []
  model_error_array = []
  tmp_weight = weight
  acc_filename = "result_data/cifar/byzantine_attacker_non_collude/acc_"+aggregation_method
  err_filename = "result_data/cifar/byzantine_attacker_non_collude/err_"+aggregation_method
...
def main(argv):
  ...
  for opt, arg in opts:
    ...
    #这是没有攻击的情况
  gf = int(f)
  fl_non_collude_byzantine_attacker(gf,round, method, int(m), int(float(r)* total_parameters), int(k), tmp_weight)
