...
def fl_non_collude_byzantine_attacker(round, gf, aggregation_method, m, r, k, model):
  model_accuracy_array = []
  model_error_array = []
  tmp_weight = model.get_weights()
  acc_filename = "result_data/byzantine_attacker_non_collude/acc_"+aggregation_method
  err_filename = "result_data/byzantine_attacker_non_collude/err_"+aggregation_method
...
def main(argv):
  ...
  if f == '0':
    #这是没有攻击的情况
    gf = int(f)
    federate_learning_without_attacker(round, method, int(m), int(float(r)* total_parameters), int(k), model_cnn)
  #如果合谋，则最后20个客户端的模型参数全部改为非常大的值在原模型
  else:
    gf = int(f)
    if collude == 'no':
      #合谋的情况下，后20个客户端为恶意的，上传的参数为非常大的值
      fl_non_collude_byzantine_attacker(round, gf, method, int(m), int(float(r)* total_parameters), int(k), model_cnn)
