...
def aggregation_sar2(model_weights, n, f, r, k, round):
  if (n - 2 * f -2 <= 0):
  ...
  if round < 300:
    ...
    return model_weights[indexs[choice]]
...
def aggregation_sar(model_weights, n, f, r, k):
    ...
    aggregation_result = []
    for l in range(layerNum):
      layer_parameters_num = len(model_weights[0][l].flatten())
      shape = model_weights[0][l].shape
      choices = np.random.choice(layer_parameters_num, int(r*layer_parameters_num))
      layers_list = [] 
      for n in range(number):
        layer = model_weights[n][l].flatten()
        sampled_layer = []
        for j in choices:
          sampled_layer.append(layer[j])
        layers_list.append(np.array(sampled_layer))
      #在sampled参数上做计算
      distance = np.zeros((number,number))
      for i in range(number):
        for j in range(i+1, number):
          distance[i][j] = np.linalg.norm(np.array(layers_list[i])-np.array(layers_list[j]))
      distance += distance.T
      #评分
      score = np.zeros((number))
      for i in range(number):
      ...
      tmp_layers = []
      for i in range(k):
        tmp_layers.append(model_weights[indexs[i]][l])
      aggregation_result.append(np.median(tmp_layers, axis=0))
...
class Network(object):
    ...
    def evaluate_backdoor(self,test_data,test_label,current_weight):
        self.model.set_weights(current_weight)
        test_error, test_acc = self.model.evaluate(test_data,test_label,verbose=0)
        return test_error, test_acc


def process_data_to_ray(images, labels, workers):
  if (len(images) != len(labels)):
    print("data doesn't match")
    return
  each_worker = (int) (len(images)/workers)
  ray_images = []
  ray_labels = []
  for i in range(workers - 1):
    tmp_images = images[i*each_worker:(i+1)*each_worker]
    tmp_labels = labels[i*each_worker:(i+1)*each_worker]
    ray_images.append(tmp_images)
    ray_labels.append(tmp_labels)
  ray_images.append(images[(workers-1)*each_worker:len(images)])
  ray_labels.append(labels[(workers-1)*each_worker:len(labels)])
  return ray_images, ray_labels

workers = 8

client_images = np.load("clients_images_backdoor_cifar.npy", allow_pickle=True)
client_labels = np.load("clients_labels_backdoor_cifar.npy", allow_pickle=True)

test_images = np.load("test_images_cifar.npy")
test_labels = np.load("test_labels_cifar.npy")
back_test_images = np.load("images_backdoor_cifar.npy",allow_pickle=True)
back_test_labels = np.load("labels_backdoor_cifar.npy",allow_pickle=True)
...
def backdoor_attacker(gf, round, aggregation_method, m, r, k, weight):
  model_accuracy_array = []
  model_error_array = []
  backdoor_accuracy_array = []
  backdoor_error_array = []
  tmp_weight = weight
  acc_filename = "result_data/cifar/backdoor/acc_"+aggregation_method
  err_filename = "result_data/cifar/backdoor/err_"+aggregation_method
  back_acc_filename = "result_data/cifar/backdoor/back_acc_"+aggregation_method+str(k)+str(250)
  back_err_filename = "result_data/cifar/backdoor/back_err_"+aggregation_method+str(k)+str(250)
  ...
  for x in range(round):
    ...
    elif (aggregation_method == "sar2"):
      tmp_weight = aggregation_sar2(weights, n, gf, r, k,x)
    del weights_list
    del weights

    res = ray.get([train_actors[i].evaluate.remote(ray_test_images[i], ray_test_labels[i], tmp_weight)  for i in range(workers)])

    back_error, back_acc = ray.get(train_actors[0].evaluate.remote(back_test_images, back_test_labels, tmp_weight))
    backdoor_accuracy_array.append(back_acc)
    backdoor_error_array.append(back_error)
    test_error, test_acc = np.mean(res, axis=0)
    print(x, test_error,test_acc)
    print(x, back_error,back_acc)
    model_error_array.append(test_error)
    model_accuracy_array.append(test_acc)
    gc.collect()
    if (x==300):
      np.save("acc_300", model_accuracy_array)
      np.save("err_300", model_error_array)
      np.save("model_weights_300", tmp_weight)
  np.save(acc_filename, model_accuracy_array)
  np.save(err_filename, model_error_array)
  np.save(back_acc_filename, backdoor_accuracy_array)
  np.save(back_err_filename, backdoor_error_array)
def main(argv):
  collude = "yes"
  method = "average"
  m = 1
  ...
  for opt, arg in opts:
    if opt in ("-f"):
      ...
      k = arg
  gf = int(f)
  backdoor_attacker(gf, round, method, int(m), float(r), int(k), tmp_weight)
